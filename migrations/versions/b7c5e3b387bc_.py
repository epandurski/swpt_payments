...
Revision ID: b7c5e3b387bc
Revises: 
Create Date: 2019-08-15 15:43:14.437055
...
def upgrade():
    ...
    sa.Column('offer_key', postgresql.BYTEA(length=12), nullable=False, comment='A random sequence of bytes. Along with `payee_creditor_id` uniquely identifies the offer. Should be impossible to guess.'),
    ...
    sa.CheckConstraint('length(offer_key) = 12'),
    sa.PrimaryKeyConstraint('payee_creditor_id', 'offer_key'),
    comment='Represents an offer to supply some goods or services for a stated price.'
    )
    op.create_table('payment_proof',
    sa.Column('payee_creditor_id', sa.BigInteger(), nullable=False, comment='The payee, also the one that is responsible to supply the goods or services.'),
    sa.Column('proof_key', postgresql.BYTEA(length=12), nullable=False, comment='A random sequence of bytes. Along with `payee_creditor_id` uniquely identifies the payment proof. Should be impossible to guess.'),
    ...
    sa.CheckConstraint('length(proof_key) = 12'),
