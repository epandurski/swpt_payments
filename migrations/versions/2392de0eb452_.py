...
Revision ID: 2392de0eb452
Revises: 
Create Date: 2019-08-15 17:24:37.488293
...
def upgrade():
    ...
    sa.Column('offer_key', postgresql.BYTEA(length=18), nullable=False),
    ...
    sa.Column('offer_request_id', sa.BigInteger(), nullable=False),
    ...
    sa.Column('offer_key', postgresql.BYTEA(length=18), nullable=False, comment='A random sequence of bytes. Along with `payee_creditor_id` uniquely identifies the offer. Should be impossible to guess.'),
    ...
    sa.CheckConstraint('length(offer_key) = 18'),
    sa.PrimaryKeyConstraint('payee_creditor_id', 'offer_key'),
    comment='Represents an offer to supply some goods or services for a stated price.'
    )
    op.create_table('paid_offer_signal',
    sa.Column('payee_creditor_id', sa.BigInteger(), nullable=False),
    sa.Column('offer_key', postgresql.BYTEA(length=18), nullable=False),
    sa.Column('payer_payment_order_id', sa.BigInteger(), nullable=False),
    sa.Column('proof_key', postgresql.BYTEA(length=18), nullable=False),
    sa.Column('payer_creditor_id', sa.BigInteger(), nullable=False),
    ...
    sa.Column('payee_creditor_id', sa.BigInteger(), nullable=False, comment='The payee, also the one that is responsible to supply the goods or services.'),
    sa.Column('proof_key', postgresql.BYTEA(length=18), nullable=False, comment='A random sequence of bytes. Along with `payee_creditor_id` uniquely identifies the payment proof. Should be impossible to guess.'),
    ...
    sa.CheckConstraint('length(proof_key) = 18'),
...
def downgrade():
    ...
    op.drop_table('paid_offer_signal')
    op.drop_table('offer')
